...
UtlBoolean SipMessage::isSameSession(const SipMessage* message) const
{
   ...
   if(message)
   {
      ...
      if(0 == localCallId.compareTo(otherCallId))
      {
         ...
         this->getFromUrl(localFromUrl);
         message->getToUrl(otherToUrl);
         this->getToUrl(localToUrl);
...
UtlBoolean SipMessage::isSameSession(Url firstUrl, Url secondUrl, UtlBoolean comparingToUrl)
{
   UtlBoolean isSame = FALSE;
   ...
   else
   {
      ...
      {
         // If the from tags are null default back to RFC 2543 comparison
         ...
         if (firstAddress.compareTo(secondAddress) == 0 &&
               ...
               (firstPort == 0 && secondPort == SIP_PORT) ||
               (firstPort == SIP_PORT && secondPort == 0)) &&
